...
| 配置键 | 环境变量 | 默认值 | 使用方 |
| --- | --- | --- | --- |
| `fuchuang.ai.python.base-url` | `FUCHUANG_AI_PYTHON_BASE_URL` | `http://127.0.0.1:8090` | `PythonRagFeedbackClient`、`PythonGoalsAdviceClient`、`PythonRoadmapRagClient` |
...
```
新增环境变量（FUCHUANG_AI_PYTHON_*） > legacy 环境变量（FUCHUANG_PYTHON_AI_*） > 专用配置键 > fuchuang.ai.python.timeout-seconds > 内置默认值
```

说明：部分历史接口只有 legacy 环境变量；若同时存在新旧环境变量，以各模块下方列出的代码实际顺序为准。Roadmap 与 Dashboard 均优先读取新增 `FUCHUANG_AI_PYTHON_*` 变量。
...
| 配置键 | 环境变量 | 默认值 | 使用方 |
| --- | --- | --- | --- |
| `roadmap.ai.base-url` | `ROADMAP_AI_BASE_URL` | `http://localhost:8090` | 历史命名，Roadmap 已改用 `fuchuang.ai.python.base-url` |
| `roadmap.ai.timeout-seconds` | `ROADMAP_AI_TIMEOUT_SECONDS` | `8` | 历史命名，Roadmap 已改用 `fuchuang.ai.python.roadmap-timeout-seconds` |
