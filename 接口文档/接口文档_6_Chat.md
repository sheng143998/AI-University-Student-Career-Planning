...
| 日期 | 变更 |
| :--- | :--- |
| 2026-06-15 | 清理旧 `ai_service/` 目录后，明确 8090 聚合服务、8091 Resume 服务、8092 Chat 服务均使用统一 `ai-service/` 代码目录，避免后续按旧目录回流。 |
...
| 2026-06-08 | 明确 Chat Python 服务入口归属：Chat 默认监听 `http://127.0.0.1:8092`。Java Chat 新增/使用 `fuchuang.ai.python.chat-base-url`，避免复用 8090 聚合入口。 |
...
| 服务目录 | 职责 | 默认地址 |
| :--- | :--- | :--- |
| `ai-service/app/main.py` | 聚合 Python AI 服务，承载 Market、Goals、Reports、Dashboard、Feedback、Roadmap 等非 Chat 能力 | `http://127.0.0.1:8090` |
| `ai-service/career_ai/resume_analysis_service.py` | Resume-AI 分析服务 | `http://127.0.0.1:8091` |
| `ai-service/app/main.py` | Chat 专属 Python RAG 边界服务，承载 `/api/v1/chat/complete` 与 `/api/v1/chat/daily-suggestions`；当前为 deterministic fallback 雏形 | `http://127.0.0.1:8092` |
...
- 端口一致性：`rg -n "8090|8091|8092|FUCHUANG_AI_PYTHON_CHAT_BASE_URL|AI_SERVICE_PORT|chat-base-url|chatBaseUrl" ai-service server/src/main/resources server/src/main/java 接口文档`
